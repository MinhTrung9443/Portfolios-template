...
fs.writeFileSync(path.join(baseDir, 'shared/utils.js'), sharedUtils);
fs.writeFileSync(path.join(baseDir, 'shared/KeyValueResidual.jsx'), keyValueResidual);
fs.writeFileSync(path.join(baseDir, 'shared/HoverTemplateTitle.jsx'), hoverTitle);

// Generate generic files for components based on extracted logical segments

const filesToCreate = {
  'personal_info/PersonalInfoBlock.jsx': `
...
export default function PersonalInfoBlock({ meta, data, accentClassName, p }) {
...
`,
  'experience/TimelineExperience.jsx': `
import { asString } from '../../parseBlockData.js';
...
`,
  'certificates/CertificatesBlock.jsx': `
import { asString } from '../../parseBlockData.js';
...
const registryFile = `
import PersonalInfoBlock from './personal_info/PersonalInfoBlock';
...
export const BlockRegistry = {
  // We can map componentTypes or full componentType_style
  
  // The generic mapping based on componentType
...
fs.writeFileSync(path.join(baseDir, '../PortfolioBlockDisplay.jsx'), portfolioBlockDisplay);
